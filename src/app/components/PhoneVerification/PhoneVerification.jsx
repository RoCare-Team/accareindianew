...
const PhoneVerification = ({ onVerificationComplete, showModal, setShowModal }) => {
  ...
  const handleVerification = async () => {
    ...
    if (otpAttempts >= 3) {
      toast.error("Cant exceed more than 3 attempts please try again after some time");
      return
    }
...
const handleResendCode = async (method) => {
  if (resendCooldown) {
    toast.warn("Please wait before resending OTP.");
    return;
  }

  try {
    // Call the same API endpoint that sends the initial OTP
    const res = await fetch("https://waterpurifierservicecenter.in/customer/ro_customer/roservice_sendotp.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ 
        phoneNumber,
        resend: true, // Optional: to indicate this is a resend request
        method: method // Optional: to specify SMS or WhatsApp
      }),
    });
    ...
    if (data.error === false) {
      ...
      // Show success toast
      toast.success(data.msg || `OTP resent via ${method === 'whatsapp' ? 'WhatsApp' : 'SMS'}`);
            ...
            <div className="text-center">
              ...
              <div className="mb-6">
                ...
                <div className='flex items-center gap-2.5 justify-center mt-2'>
                  ...
                  >
                    WhatsApp {resendCooldown ? `(${resendTimer}s)` : ''}
                  </button>
                  <button
                    ...
                    disabled={resendCooldown}
                  >
                    SMS {resendCooldown ? `(${resendTimer}s)` : ''}
                  </button>
                </div>
              </div>
